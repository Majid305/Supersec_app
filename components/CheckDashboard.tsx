...
export const CheckDashboard = ({ onEdit }: { onEdit: (check: RejectedCheck) => void }) => {
    ...
    const handleDownloadPDF = async (check: RejectedCheck, e: React.MouseEvent) => {
        ...
        try {
            ...
            // Header
            doc.setFillColor(132, 204, 22); // Citron Vert
            ...
            // LONG RIB FIELD - LAST AND FULL WIDTH
            doc.setFontSize(12);
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(100);
    ...
    const getSituationBadge = (sit: CheckSituation) => {
        const colors: Record<string, string> = {
            [CheckSituation.COFFRE]: 'bg-gray-100 text-gray-700',
            [CheckSituation.REGULARISE]: 'bg-green-100 text-green-700',
            [CheckSituation.CONTENTIEUX]: 'bg-red-100 text-red-700',
            [CheckSituation.RESTITUE]: 'bg-blue-100 text-blue-700',
        };
    ...
    const getStatusIndicator = (check: RejectedCheck) => {
        ...
        if (check.date_heure_rappel) {
            ...
            const now = Date.now();
            if (deadline < now) return 'border-l-4 border-red-600 animate-pulse'; // Retard critique
            if ((deadline - now) < 86400000) return 'border-l-4 border-orange-500'; // Moins de 24h
        }
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24">
                ...
                {filtered.map(check => (
                    <div 
                        ...
                        className={`bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm cursor-pointer active:scale-[0.98] transition-all relative group ${getStatusIndicator(check)}`}
                    >
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-50 dark:border-slate-800 flex justify-between items-end">
                            ...
                            <div className="text-right">
                                <p className="text-[10px] text-gray-400 font-mono font-bold">N° {check.numero_cheque}</p>
                                <p className="text-[10px] text-gray-400 uppercase font-bold">{check.date_rejet}</p>
                            </div>
                        </div>

                        {menuOpenId === check.id && (
                            <div className="absolute right-4 top-12 bg-white dark:bg-slate-800 shadow-2xl border border-gray-100 dark:border-slate-700 rounded-2xl z-30 flex flex-col w-48 py-2 animate-in fade-in zoom-in-95 duration-150">
                                <button onClick={() => { onEdit(check); setMenuOpenId(null); }} className="flex items-center px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-700 text-sm font-bold text-gray-700 dark:text-gray-200 transition-colors">
                                    <Edit size={16} className="mr-3 text-citron" /> Modifier
                                </button>
                                <button onClick={(e) => { handleDuplicate(check, e); setMenuOpenId(null); }} className="flex items-center px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-700 text-sm font-bold text-gray-700 dark:text-gray-200 transition-colors">
                                    <Copy size={16} className="mr-3 text-blue-500" /> Dupliquer
                                </button>
                                <button onClick={(e) => { handleDownloadPDF(check, e); setMenuOpenId(null); }} className="flex items-center px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-700 text-sm font-bold text-gray-700 dark:text-gray-200 transition-colors">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); setMenuOpenId(null); }} className="absolute -top-2 -right-2 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 p-1.5 rounded-full shadow-lg">
